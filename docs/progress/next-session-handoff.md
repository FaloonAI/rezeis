...
Generated: 2026-06-03T21:49:34.174Z (checks not re-run; previous observed results preserved)
...
Current recommendation: move to the P2 frontend correctness baseline, starting with auth readiness and query-key/realtime invalidation consistency for payment/admin surfaces. The S7/S5 payment transaction slice is closed for now: `AdminPaymentTransactionsController` has explicit `RbacGuard` + `payments:view/create` metadata, the payments nav/page/quick-search no longer expose payment surfaces without the matching grants, and the legacy payments webhook tab now uses the current `/admin/payments/webhooks/events` contract. `gatewayId` and `providerEventId` remain operational identifiers available only behind explicit payment/webhook RBAC, not a separate reveal flow. Do not globalize `RbacGuard` yet.
...
Recommended first slice: begin P2 frontend correctness by auditing auth readiness and query-key/realtime invalidation consistency for admin surfaces, especially payments and other recently RBAC-gated pages. Do not globalize `RbacGuard` yet; revisit API-token per-audience scopes only if a concrete client split appears.
