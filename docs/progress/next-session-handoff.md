...
Generated: 2026-06-03T20:40:08.582Z (checks not re-run; previous observed results preserved)
...
Current recommendation: continue S7 Payment Diagnostics Sanitization by auditing transaction/gateway diagnostic response masking and user-facing/internal payment status payloads. Webhook raw payload reveal now returns redacted payloads while preserving audit logging, and webhook ingress responses no longer echo raw webhook bodies. Payment gateway registry RBAC is closed for the current admin surface. Do not globalize `RbacGuard` yet.
...
- Continue S7 Payment Diagnostics Sanitization: shared payment diagnostic message redaction now covers auth headers, cookies, provider/gateway ID assignments, and URL assignments; `gatewayData.providerResponse` is now redacted before persistence, webhook ops are RBAC-gated, webhook detail `includeRaw=true` returns redacted payload while preserving audit logging, public/internal webhook ingress responses no longer echo raw webhook bodies, and the credential-bearing payment gateway settings UI/API surface is now behind explicit `payment_gateways:view/edit`. Continue auditing transaction/gateway diagnostic response masking and user-facing/internal payment status payloads.
